<template>
  <el-aside width="200px" style="background-color: #0c4f9c; color: aliceblue">
    <el-scrollbar>
      <div style="font-size: 20px; font-weight: bold; margin-bottom: 0px; margin-left: 30px" class="ti">智慧睡眠</div>
      <!--居中-->
      <div style="font-size: 16px; margin: 10px; margin-top: 0px">睡眠分期和评估系统</div>
      <div>
        <el-menu :default-active="route.fullPath" :router="true" style="margin: 0px" background-color="#155398" text-color="white">
          <template v-for="data in datalist" :key="data.path">
            <div>
              ...
              <!--单一列表-->
              <el-menu-item :index="data.path" v-else-if="checkAuth(data.path)" style="color: white">
                <el-icon>
                  <component :is="mapIcons[data.icon]"></component>
                </el-icon>
                <span>{{ data.title }}</span>
...
<script setup>
import { HomeFilled, Key, OfficeBuilding, UploadFilled, List, User, Aim, DocumentAdd, Reading, Help } from '@element-plus/icons-vue'
